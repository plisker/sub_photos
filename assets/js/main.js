...
$(document).ready(function () {
  // Wait for Font Awesome to load and convert icons to SVGs
  function waitForFontAwesome() {
    // Check if Font Awesome has converted the icons to SVGs
    var checkInterval = setInterval(function () {
      var logo = document.getElementById("fh5co-logo");
      if (logo && logo.querySelector("svg.svg-inline--fa")) {
        logo.classList.add("fa-loaded");
        clearInterval(checkInterval);
      }
    }, 10);

    // Failsafe: show icons after 500ms even if Font Awesome hasn't loaded
    setTimeout(function () {
      var logo = document.getElementById("fh5co-logo");
      if (logo) {
        logo.classList.add("fa-loaded");
